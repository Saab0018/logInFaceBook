...
.controller('AppCtrl', function ($scope, $ionicModal, $state, $timeout, $cordovaOauth, $location, $localStorage) {
  ...
  $scope.logout = function () {
    ...
    //    $state.reload();    //start login function
    $scope.login();
  };

  $scope.facebookLogin = function () {
    $cordovaOauth.facebook(1829237564030234, ["email"]).then(function (result) {
      // results
      ...
      console.log("im past the state go");
...
.controller('ProfileController', function ($scope, $state, $http, $localStorage, $location) {
  ...
  //  $scope.init = function () {
  console.log("Profile: " + $localStorage.accessToken);
